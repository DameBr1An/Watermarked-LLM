...
class WatermarkBase:
    def __init__(
        self,
        vocab: list[int] = None,
        gamma: float = 0.5,     # green list size,
    ...
    def _seed_rng(self, input_ids: torch.LongTensor, seeding_scheme: str = None) -> None:
        ...
        if seeding_scheme == "simple_1":
            ...
            prev_token = input_ids[-1].item()   # Apply the language model to prior tokens s(−Np)· · · s(t−1) to get a logit vector l(t) over the vocabulary
            self.rng.manual_seed(self.hash_key * prev_token)    #Compute a hash of token s(t−1), seed a random number generator.
        else:
...
class WatermarkLogitsProcessor(WatermarkBase, LogitsProcessor):
    ...
    def _bias_greenlist_logits(self, scores: torch.Tensor, greenlist_mask: torch.Tensor, greenlist_bias: float) -> torch.Tensor:
        # Add δ to each green list logit.
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        if self.rng is None:    # random number generator
